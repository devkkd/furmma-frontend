// data/dummyOrders.js

export const dummyOrders = [
  {
    _id: "ORD001",
    user: "65abc123user",
    items: [
      {
        product: {
          _id: "prod001",
          name: "Canine Creek Club Ultra Premium Dry Dog Food for All Lifestages, 20 kg Pack",
          image: "/images/Products/p8.png",
        },
        quantity: 1,
        price: 2449,
      },
    ],
    totalAmount: 2229,
    shippingAddress: {
      street: "123, Arera Colony",
      city: "Bhopal",
      state: "Madhya Pradesh",
      zipCode: "462016",
      phone: "9876543210",
    },
    paymentMethod: "upi",
    paymentStatus: "paid",
    orderStatus: "delivered",
    trackingNumber: "TRK100001",
    createdAt: "2025-11-22T10:45:00Z",
  },

  {
    _id: "ORD002",
    user: "65abc123user",
    items: [
      {
        product: {
          _id: "prod002",
          name: "Pedigree Adult Dry Dog Food – Chicken & Vegetables, 10 kg",
          image: "/images/Products/p5.png",
        },
        quantity: 2,
        price: 1299,
      },
    ],
    totalAmount: 2498,
    shippingAddress: {
      street: "45, Vijay Nagar",
      city: "Indore",
      state: "Madhya Pradesh",
      zipCode: "452010",
      phone: "9123456789",
    },
    paymentMethod: "card",
    paymentStatus: "paid",
    orderStatus: "shipped",
    trackingNumber: "TRK100002",
    createdAt: "2025-12-03T14:20:00Z",
  },

  {
    _id: "ORD003",
    user: "65abc123user",
    items: [
      {
        product: {
          _id: "prod003",
          name: "Drools Focus Puppy Super Premium Dry Food, 4 kg",
          image: "/images/Products/p3.png",
        },
        quantity: 1,
        price: 799,
      },
    ],
    totalAmount: 799,
    shippingAddress: {
      street: "78, MG Road",
      city: "Pune",
      state: "Maharashtra",
      zipCode: "411001",
      phone: "9988776655",
    },
    paymentMethod: "cash",
    paymentStatus: "pending",
    orderStatus: "confirmed",
    trackingNumber: null,
    createdAt: "2025-12-15T09:10:00Z",
  },
];


export const statusMap = {
  pending: 'Pending',
  confirmed: 'Confirmed',
  processing: 'Ongoing',
  shipped: 'Shipped',
  delivered: 'Delivered',
  cancelled: 'Cancelled',
}

